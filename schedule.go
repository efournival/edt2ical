...
var (
	...
	isGarbage          = regexp.MustCompile(`(?i)(matin|midi|semaine|date|conges|page|réserve|reserve|lundi|ascension|^\d{2}\s\d{2})`)
	matchGroup         = regexp.MustCompile(`(?i)\s*Gr\.*\s*(\d{1})\s*`)
	matchGroupLocation = regexp.MustCompile(`(?i)\s*Gr\.*\s*(\d{1})[:-|\s]*(salle|\s*)([A-Z]{1}\s*\d{3})\s*`)
	...
	matchLocation      = regexp.MustCompile(`(?i)((Salle\s*:*\s*)*([A-Z]{1}\s*\d{3})|\w*[\s|\-]*amphi|Bat\.\s*\d{3})`)
